...
class JsonKeyValueWidget(BaseJsonKeyValueWidget):
    ...
    def get_context(self, name: str, value: Any, attrs: dict[str, Any] | None) -> dict[str, Any]:
        ...
        context["widget"]["display_classes"] = self.get_display_classes(extra_classes=("core-json-key-value__display",))
    ...
    def build_rows(self, value: Any) -> list[dict[str, str]]:
        ...
        if isinstance(value, list | tuple):
            rows = [self.build_row_from_submitted_value(row) for row in value]
            return rows or [{"key": "", "value": ""}]

        if isinstance(value, dict):
            rows = [{"key": str(key), "value": str(item_value)} for key, item_value in value.items()]
            return rows or [{"key": "", "value": ""}]
