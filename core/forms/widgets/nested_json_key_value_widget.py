...
class NestedJsonKeyValueWidget(BaseJsonKeyValueWidget):
    ...
    def get_context(self, name: str, value: Any, attrs: dict[str, Any] | None) -> dict[str, Any]:
        ...
        if is_readonly and self.has_only_empty_row(rows):
            rows = [
                self.build_row(row_id="row_1", parent_id="", key="-", value="-", depth=1),
            ]
        ...
        context["widget"]["display_classes"] = self.get_display_classes(
            extra_classes=("core-nested-json-key-value__display",)
        )
    ...
    def has_only_empty_row(self, rows: list[dict[str, Any]]) -> bool:
        """Return whether rows represent one empty nested placeholder.
        ...
        row = rows[0]
        return not row.get("key") and not row.get("value") and not row.get("parent_id")
    ...
    def build_rows_from_submitted_rows(self, value: list[Any] | tuple[Any, ...]) -> list[dict[str, Any]]:
        ...
        """
        parsed_rows = [self.parse_submitted_row(row, index=index) for index, row in enumerate(value)]
        if not parsed_rows:
            return [self.build_row(row_id="row_1", parent_id="", key="", value="", depth=1)]
